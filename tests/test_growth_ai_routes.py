...
class GrowthAIRouteTests(unittest.TestCase):
    ...
    def test_cached_result_renders_without_ids(self):
        ...
        self.assertIn('세부 관찰', body)
        self.assertIn('지금 해볼 일', body)
        self.assertIn('다음에 확인할 점', body)
        self.assertIn('가장 먼저 볼 변화', body)
        self.assertIn('왜 중요하게 보나요?', body)
        self.assertIn('growth-ai-priority', body)
        self.assertIn('growth-ai-action-card', body)
        self.assertIn('growth-ai-check', body)
        self.assertIn('data-ai-role="observations-fold"', body)
        self.assertIn('data-ai-role="next-check-list"', body)
        fold = body.split('data-ai-role="observations-fold"', 1)[1].split('</details>', 1)[0]
        self.assertNotIn(' open', fold[:80])
