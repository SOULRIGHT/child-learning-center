...
class GrowthAiResultUiTests(unittest.TestCase):
    def test_result_hierarchy_and_js_fillers(self):
        ...
        self.assertIn('growth_teacher_prompt_v7', (PROJECT_ROOT / 'features' / 'growth' / 'ai' / 'prompt.py').read_text(encoding='utf-8'))
        self.assertIn('최대 2문장', (PROJECT_ROOT / 'features' / 'growth' / 'ai' / 'prompt.py').read_text(encoding='utf-8'))
        self.assertIn('서로 다른 단위의 정량 비교', (PROJECT_ROOT / 'features' / 'growth' / 'ai' / 'prompt.py').read_text(encoding='utf-8'))
        self.assertIn('이전 기록 기준 해석', html)
        self.assertIn('이 해석 이후 기록이 변경됐어요.', html)
        self.assertIn('현재 기록과 내용이 다를 수 있습니다.', html)
        self.assertIn('현재 기록으로 다시 분석', html)
